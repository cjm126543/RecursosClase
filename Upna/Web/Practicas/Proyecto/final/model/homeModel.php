...
class homeModel
{
    ...
    {
        require_once("/var/www/html/final/config/credentials.php");
        // require_once('C:\xampp\htdocs\proyecto_siw\final\config\credentials.php');
        $cred = new credentials();
        $this->accessToken = $cred->getToken();
        require_once("/var/www/html/final/config/db.php");
        // require_once('C:\xampp\htdocs\proyecto_siw\final\config\db.php');
        $pdo = new db();
        $this->PDO = $pdo->conexion();
        $this->inicializarLineas();
        $this->creaTablaUsuarios();
    }

    public function creaTablaUsuarios() {
        $statement = $this->PDO->prepare("CREATE TABLE IF NOT EXISTS final_usuarios
            (
    ...
    {
        $statement = $this->PDO->prepare("INSERT INTO final_usuarios values(null,:correo,:password)");
    ...
    {
        $statement = $this->PDO->prepare("SELECT password FROM final_usuarios WHERE correo = :correo");
        $statement->bindParam(":correo", $correo);
    ...
    public function inicializarLineas() {
        ...
        $statement = $this->PDO->prepare("CREATE TABLE IF NOT EXISTS final_lineas
            (
        ...
        $statement = $this->PDO->prepare("INSERT IGNORE INTO final_lineas VALUES(null, :linea, :label, :nameA, :nameB)");
        for ($i = 0; $i < sizeof($obj->data); $i++) {
    ...
    public function seleccionaAllLineas() {
        $statement = $this->PDO->prepare("SELECT * FROM final_lineas");
        try {
            $statement->execute();
        } catch (PDOException $e) {
    ...
    public function seleccionaLinea() {
        $lineLabel = $_POST['labelLinea'];
        $statement = $this->PDO->prepare("SELECT linea from final_lineas 
                                            WHERE label = '" . $lineLabel . "'");
        try {
            $statement->execute();
        } catch (PDOException $e) {
            echo "No se ha podido seleccionar la linea";
            return false;
        }
