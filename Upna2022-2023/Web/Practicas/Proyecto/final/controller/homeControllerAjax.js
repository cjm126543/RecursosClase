class homeControllerAjax {
  ...
  setLinea() {
    ...
    let texto_linea =
      lin.options[lin.selectedIndex].text.match(/: (.+?) \/ (.+)/);
    document.getElementById("ida").textContent =
      "Hacia: " + texto_linea[2].trim();
    document.getElementById("vuelta").textContent =
      "Hacia: " + texto_linea[1].trim();
  }
  ...
  dibujaParadas(paradas, col) {
    ...
    for (var i = 0; i < paradas.length; ++i) {
      ...
      let pop = marcador.bindPopup(
        "P" +
          (i + 1) +
          ": " +
          name +
          "\n" +
          '<a id="show-hide" href="#" onclick="homeControllerJs.getStopInfo(' +
          paradas[i].stop +
          ", " +
          markerId +
          ');">Ver mas</a>'
    ...
    let grafo = L.polyline(listaParadas, { color: col, weight: 3 }).addTo(
      this.map
    );
  ...
  async getRoute() {
    ...
    const res = await $.ajax({
      url:
        "https://openapi.emtmadrid.es/v1/transport/busemtmad/lines/" +
        lineaId +
        "/stops/" +
        this.direccion +
        "/",
  ...
  async getStopInfo(stopId, markerId) {
    ...
    for (var j = 0; j < labelArr.length; j++)
      popupContent +=
        "<br>" + labelArr[j] + " " + headerA_arr[j] + " / " + headerB_arr[j];
    ...
    if (popup.isOpen()) {
      ...
      if (anchor.innerText == "Ver mas") {
        ...
        anchor.setAttribute(
          "onclick",
          "homeControllerJs.cleanPopUp(" + markerId + ", " + stopId + ");"
        );
  ...
  cleanPopUp(markerId, stopId) {
    ...
    let popupContent = popup.getContent();
    popup.setContent(
      popupContent.substring(
        0,
        popupContent.indexOf("Ocultar") + "Ocultar".length
      )
    );
    let tempElement = document.createElement("div");
    tempElement.innerHTML = popup.getContent();
    ...
    if (anchor.innerText == "Ocultar") {
      ...
      anchor.setAttribute(
        "onclick",
        "homeControllerJs.getStopInfo(" + stopId + ", " + markerId + ");"
      );
      popup.setContent(tempElement.innerHTML);
    }
  }
