class lineasControllerAjax {
    ...
    async showLinea(identificador) {
        ...
        const res = await $.ajax({
            url: "https://openapi.emtmadrid.es/v1/transport/busemtmad/lines/incidents/" + identificador + "/",
            method: "GET",
            headers: {
                'accessToken': token
            },
        ...
        var endingSentence = "Ver más detalle en documento adjunto.<p><img src='http://feeds.emtmadrid.es:8082/images/00-Logo-RSS_Corporativo.png'/></p>"; 
        for (var i = 0; i < res.data[0].item.length; i++) {
            ...
            if (descripcion.endsWith(endingSentence)) descripcion = descripcion.slice(0, -endingSentence.length);
